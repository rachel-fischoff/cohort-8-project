...
//GET route for /userId/groups
app.get('/groups/:groupID', isLoggedIn, (req, res, next) => {
  Groups.findByIdAndUpdate(req.params.group, {company_name: req.company.name, description: req.company.description }, function(err, group){
    if (err) {
        return next(err)
    }if(todo) {
   res.send(todo)
    ...
    res.status(404);
    return res.end(`Group with id ${req.params.group} not found`);
}
});
})
